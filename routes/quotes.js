  ...
  server.route([
    {
      ...
      handler: function (request, reply) {
        reply('Hello TROLOLOLOLOLOLOLOLOLOLOL Harry Quotes~~~')
      }
    },
    //GETS ALL QUOTES
    { 
    ...
    },
    {
      ...
      handler: function(request,reply) {
        ...
        if (Number.isNaN(id)) {
          return reply('GIVE ME A NUMBER').code(404);
        }
      reply( db[id] );
      }
    },
    ...
    },
    {
     ...
     handler: function(request, reply) {
       var id = Number(request.params.id);
       db.splice(id,1);
       reply("Quote: " + id + " has been Deleted.");
     }
    },
    { 
      method:'GET',
      path: '/quotes/random',
      handler: function(request, reply) {
        var randomNum = Math.round(Math.random()*db.length);
